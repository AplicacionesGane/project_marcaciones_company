    ...
    axios.post(`${URL_API}/cargo`, { codigo, descripcion: nombreCargo })
      .then(response => {
        ...
        if (response.status === 201) {
          toast({ title: 'El cargo se creo correctamente', description: 'cargo creada' })
          setCodigo('')
          setNombreCargo('')
          setRequest(true)
        }
      ...
      .catch(error => {
        console.log(error)
        toast({ title: error.response.data.message || 'Error ', description: 'Error al crear el cargo' })
      })
  }
    ...
    axios.put(`${URL_API}/updatecargo`, { id, codigo, nombre: nombreCargo })
      .then(response => {
        if (response.status === 200) {
          toast({ title: 'El área se actualizó correctamente', description: 'Área actualizada' })
          setCodigo('')
          setNombreCargo('')
          setRequest(true)
      ...
      .catch(error => {
        console.log(error)
        toast({ title: error.response.data.message || 'Error ', description: 'Error al actualizar el área' })
      })
  }
  ...
  const confirmDeletecargo = () => {
    if (cargoDelete !== null) {
      axios.delete(`${URL_API}/cargo/${cargoDelete}`)
        .then(response => {
          if (response.status === 200) {
            toast({ title: 'El cargo se eliminó correctamente', description: 'cargo eliminada' });
            setRequest(true);
          }
        })
        .catch(error => {
          console.log(error);
          toast({ title: error.response?.data?.message || 'Error', description: 'Error al eliminar el cargo' });
        })
        .finally(() => {
          closeModal();
        });
