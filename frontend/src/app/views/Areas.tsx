    ...
    axios.post(`${URL_API}/area`, { codigo, descripcion: nombreA })
      .then(response => {
        ...
        if (response.status === 201) {
          toast({ title: 'El área se creo correctamente', description: 'Área creada' })
          setCodigo('')
          setNombreA('')
          setRequest(true)
        }
      ...
      .catch(error => {
        console.log(error)
        toast({ title: error.response.data.message || 'Error ', description: 'Error al crear el área' })
      })
  };
  ...
  const confirmDeleteArea = () => {
    if (areaToDelete !== null) {
      axios.delete(`${URL_API}/area/${areaToDelete}`)
        .then(response => {
          if (response.status === 200) {
            toast({ title: 'El área se eliminó correctamente', description: 'Área eliminada' });
            setRequest(true);
          }
        })
        .catch(error => {
          console.log(error);
          toast({ title: error.response?.data?.message || 'Error', description: 'Error al eliminar el área' });
        })
        .finally(() => {
          closeModal();
        });
